export function getConfetti() {
  ...
  function startConfetti(timeout: any, min: number, max: number) {
    ...
    if (canvas === null) {
      ...
      canvas.setAttribute('id', 'confetti-canvas-' + randomId)
      canvas.setAttribute(
        'style',
  ...
  function updateParticles() {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (
        ...
        if (
          streamingConfetti &&
          particles.length <= confetti.maxCount
        ) {
